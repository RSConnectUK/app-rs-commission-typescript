...
const TrakScreenContainer = (props: any) => {
  ...
  const loadJobs = async () => {
    setLoading(true);
    const data = await getAssociated(`TRAK`);
    setJobs(data);
    setLoading(false);
    setMenuOpened(true);
  }
  ...
  const getButtons = () => {
    ...
    Object.entries(jobs).map((job: any) => buttons.push({text: job[1].reg, handler: () => {
      ...
      updatedForm.car_reg = job[1].reg;
      updatedForm.car_make = job[1].make;
      updatedForm.car_model = job[1].model;
  ...
  return <React.Fragment>
    ...
    <IonGrid>
      <IonRow className="ion-justify-content-center">
        ...
        {props.loadingLocally && <IonLoading isOpen={true} message={'Loading Autofill...'} />}
          {
            props.children
          }
        </IonCol>
